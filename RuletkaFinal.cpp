...
struct Gra {
    ...
    const int naboje[4] = {0, 4, 6, 8};
    const double prawdziweNaboje[4] = {0, 0.5, 0.66, 0.75};
...
int turaKomputera(Gracz &komputer, Gracz &czlowiek, Gra &ruletka, int runda) {
    if (komputer.zycia > 0 && ruletka.indeksNaboju <= ruletka.naboje[runda]) {
        ...
        if (losowyWybor == 1) {
            komputer.strzel(&czlowiek, ruletka.bron[ruletka.indeksNaboju - 1]);
            ...
            komputer.strzel(&komputer, ruletka.bron[ruletka.indeksNaboju - 1]);
            ruletka.indeksNaboju++;
            if (komputer.zycia <= 0) {
                cout << "Gracz " << czlowiek.nazwa << " wygrywa runde " << runda << ".\n";
                return 2; // Koniec rundy, nie gry
            }
            // W przypadku strzału do siebie, komputer kontynuuje turę
        }
    }
...
void graj() {
    ...
    for (int runda = 1; runda <= 3; ++runda) {
        ...
        for (int i = 0; i < ruletka.naboje[runda]; i++) {
            ...
            if (cin >> wybor) {
                if (wybor == 1) {
                    czlowiek.strzel(&komputer, ruletka.bron[ruletka.indeksNaboju - 1]);
                    if (komputer.zycia <= 0) {
                    ...
                    czlowiek.strzel(&czlowiek, ruletka.bron[ruletka.indeksNaboju - 1]);
                    if (ruletka.bron[ruletka.indeksNaboju - 1] == 1) {
